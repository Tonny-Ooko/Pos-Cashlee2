...
google = oauth.remote_app(
    'google',
    consumer_key='822402527612-jqbrumfeup205aabomol0hibeq73n4vi.apps.googleusercontent.com',
    consumer_secret='GOCSPX-WQ7JECy4Hf0Q9lPIl9oSVYfoENV',
...
class PointOfSale(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    # Add fields related to point of sale
...
def generate_report():
    ...
    sales_data = db.session.query(PaymentTransaction.date, PaymentTransaction.amount,
                                  StoreManagement.name, PointOfSale.name, PointOfSale.price) \
        .join(StoreManagement, StoreManagement.id == PaymentTransaction.store_id) \
        .join(PointOfSale, PointOfSale.id == PaymentTransaction.pos_id) \
...
def forgot_password():
    if request.method == 'POST':
        # Check if the user is already authorized with Google
        if not google.authorized:
            return redirect(url_for('login4'))
            # Send a password reset email using OAuth2 authentication
            if not google.authorized:
                return redirect(url_for('google.login'))
            from sqlalchemy.dialects.mssql import json
            response = google.post('https://www.googleapis.com/gmail/v1/users/me/messages/send',
                                  headers={'Content-Type': 'application/json'},
                                  data=json.dumps({
                                      'raw': 'BASE64_ENCODED_MESSAGE'  # Replace with your email content
                                  }))
...
def register():
    ...
    if not password_pattern.match(password):
        flash("Password must be at least six characters long and contain at least one uppercase letter, one lowercase letter, and one digit.", "error")
        return redirect(url_for("index"))
    ...
    if not User.query.filter_by(username=username).first():
        ...
        new_user = User(username=username, email=email, password_hash=hashed_password, role=role)  # Update 'password' to 'password_hash'
